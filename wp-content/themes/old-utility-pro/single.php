...
//* Customize the header post meta function

add_action( 'genesis_entry_header', 'genesis_header_post_meta', 1 );

function genesis_header_post_meta($post_meta) {
	
	echo '<p class = "header-entry-meta">';
...
echo '<div class="embed-container"><h3>Context:</h3>';
...
function disqusstart() {
	echo '<div id = "disquscommentsarea" style = "background: #f9f9f9; padding: 24px;">';
}
function disqusend() {
	echo '</div>';
}
...
add_action( 'genesis_after_entry' , 'disqusend', 15);
...
add_action( 'genesis_after_entry' , 'relatedposts', 4);
